...
while read -r line; do
  ...
  if ! tang_servers=$(clevis luks list -d "/dev/${device}1" 2>/dev/null ); then
    echo "${device}1 : Warning: issue readling luks list for device, disk not encrypted?" >&2
    continue
...
============================================================================================='
    ...
    for device in $devices_to_do; do
        ...
        echo -n 'Enter your existing LUKS password: '
 ...
 echo "Placeholder keyfile already in place, skipping"
  ...
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if ! mv /root/keyfile /root/keyfile.backup; then
      ...
      echo "Old encryption key stored in '/root/keyfile.backup', ensure you have a backup before deleting this file!"
    fi
